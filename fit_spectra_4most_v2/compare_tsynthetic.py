...
if __name__ == '__main__':
    ...
    snr_to_do = 1000
    payne_data = pd.read_csv(f"fitted_{snr_to_do}.csv")
    ...
    for i, element_to_fit in enumerate(elements_to_fit):
        ...
        diff = merged_data[element_to_fit.replace("_y", "_x")] + merged_data["feh_x"] - (merged_data[element_to_fit] + merged_data["feh_y"])
    ...
    for i, element_to_fit in enumerate(elements_to_fit):
        ...
        ax[row, col].scatter(merged_data[element_to_fit.replace("_y", "_x")] + merged_data["feh_x"],
            merged_data[element_to_fit.replace("_y", "_x")] + merged_data["feh_x"] - (merged_data[element_to_fit] + merged_data["feh_y"]),
            c=plot_color_column, cmap=cmap, s=14
        )

        ax[row, col].set_xlabel(f"{element_to_fit.replace('_Fe_y', '_H')} (True)")
        ax[row, col].set_ylabel(f"{element_to_fit.replace('_Fe_y', '_H')} (True - Fitted)")
