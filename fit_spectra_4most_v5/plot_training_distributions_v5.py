...
MAX_COUNT = 1000    # bins with > MAX_COUNT will use the max colour
...
if __name__ == '__main__':
    ...
    axs[2].hist(data_all['vmic'], bins=15, range=(np.min(data_all['vmic']), np.max(data_all['vmic'])), histtype='bar', color='gray', alpha=0.6, density=False, rasterized=True)

    axs[2].text(0.07, 0.21, "vmic [km/s]", transform=axs[2].transAxes, ha='left', va='top', fontsize=16, color='k')
    ...
    # elements start at column 6 in your file
    for i, element in enumerate(data_all.columns[6:]):
        # raw 2D histogram in *counts*

        axs[i + 3].hist(data_all[element], bins=15, range=(np.min(data_all[element]), np.max(data_all[element])), histtype='bar', color='gray', alpha=0.6, density=False, rasterized=True)
        ...
        # mask low-occupancy bins to force white, and clip very high bins
        counts = np.where(counts < MIN_COUNT, np.nan, counts)
        counts = np.clip(counts, None, MAX_COUNT)

        X, Y = np.meshgrid(xedges, yedges)
        im = axs[i + 3].pcolormesh(
            X, Y, counts.T,
            cmap=cmap, norm=norm,
            shading='auto',
            rasterized=True
        ...
        axs[i + 3].text(0.07, 0.21, text, transform=axs[i + 3].transAxes,
                        ha='left', va='top', fontsize=16, color='black')

        if dist_width <= 1.8:
            axs[i + 3].xaxis.set_major_locator(MultipleLocator(0.5))
            axs[i + 3].xaxis.set_minor_locator(MultipleLocator(0.1))
        else:
            axs[i + 3].xaxis.set_major_locator(MultipleLocator(1))
            axs[i + 3].xaxis.set_minor_locator(MultipleLocator(0.2))

        #axs[i + 3].set_xlim(-5.1, 0.51)
        axs[i + 3].set_xlim(np.min(data_all[element]) - 0.06,
                            np.max(data_all[element]) + 0.06)

    # --- make a single colourbar on the right for all subplots -----------
    #cbar = fig.colorbar(im, ax=axs.reshape(-1), pad=0.02)  # uses shared norm/cmap
    #cbar.set_label("Count per bin")

    # --- layout/ticks tweaks (unchanged logic) ---------------------------
    ...
    fig.supxlabel('[X/Fe]', y=0.05, fontsize=18)
