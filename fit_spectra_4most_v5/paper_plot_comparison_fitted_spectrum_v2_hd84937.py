...
# and 5242.49
    ...
    #(major_x, minor_x, major_y, minor_y)
    zoom_regions = [
        [(4034.483 - 2, 4034.76 + 0.5), (0.785, 1.02), (1, 0.1, 0.05, 0.01)], # Mn
        #(5166.51, 5184.49),
        #[(5166.51, 5174.5), (0.47, 1.02), (2, 0.5, 0.1, 0.02)],
        [(5263 - 2, 5263 + 3), (0.92, 1.01), (1, 0.25, 0.02, 0.005)],
        #(5166.71, 5168.00),
        [(6707.21, 6708.349), (0.82, 1.03), (0.5, 0.1, 0.05, 0.01)],  # Li line
        [(4214.9, 4216.499), (0.6, 1.02), (0.5, 0.1, 0.1, 0.025)],  # Sr
        [(5210.390 - 0.7, 5210.390 + 0.7), (0.95, 1.01), (0.5, 0.1, 0.01, 0.005)],  # Ti I
        #(5264.2 - 0.5, 5264.2 + 0.5),  #
        # Add more regions as needed
        #[(6555.1, 6570), (0.17, 1.005), (5, 1, 0.2, 0.05)],
        [(6456.4 - 1, 6456.4 + 1), (0.975, 1.005), (0.5, 0.1, 0.01, 0.005)],
    ]

    lines_to_highlight = {"z1": {"Mn I": [4033.062, 4034.483], "Fe I": [4032.63]},
                          "z2": {"Ca I": [5261.70, 5262.24, 5264.24, 5265.56], "Ti II": [5262.14], "Fe I": [5263.31], "Cr I": [5264.13]},
                          "z3": {"6Li": [6707.921, 6708.072], "7Li": [6707.764, 6707.915]},
                          "z4": {"Sr II": [4215.52], "Fe I": [4216.18]},
                          "z5": {"Ti I": [5210.390]},
                          "z6": {"Fe II": [6456.38], "Ca I": [6455.60]}}
    ...
    titles = ['Mn I', 'Ca I', 'Li I doublet', 'Sr II', 'Ti I', 'Fe II']
    ...
    for i, ax_key in enumerate(['z1', 'z2', 'z3', 'z4', 'z5', 'z6']):
        ...
        if i < len(zoom_regions):
            zr0, zr1 = zoom_regions[i][0]
            y_min, y_max = zoom_regions[i][1]
            major_x, minor_x, major_y, minor_y = zoom_regions[i][2]
            # Mask zoom region
            ...
            # --- major ticks every 10 on x, every 5 on y ---
            axz.xaxis.set_major_locator(MultipleLocator(major_x))  # dx = 10
            axz.yaxis.set_major_locator(MultipleLocator(major_y))  # dy = 5

            # (optional) add minor ticks every 2 on x and 1 on y
            axz.xaxis.set_minor_locator(MultipleLocator(minor_x))  # dx = 2
            axz.yaxis.set_minor_locator(MultipleLocator(minor_y))
    ...
    plt.savefig("../plots/HD84937_4MOST-HR_Payne_fit_v2.pdf", bbox_inches='tight')
    plt.show()