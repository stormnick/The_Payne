...
if __name__ == '__main__':
    ...
    axs[0].scatter(feh_plot, ali_payne, s=20, edgecolor='none', c='k')
    axs[0].errorbar(feh_plot, ali_payne, xerr=xerr_plot, yerr=yerr_plot, fmt='none', ecolor='k', elinewidth=0.5, capsize=2)
    axs[0].set_ylabel(r"A(Li)", fontsize=fontsize)
    axs[0].set_xlim(-4.0, 0.4)
    axs[0].set_ylim(0.0, 3.5)
    axs[0].set_xlabel(r"[Fe/H]", fontsize=fontsize)
    ...
    for color, curve, label, linestyle in zip(colors, curves, curve_labels, linestyles):
        axs[0].plot(curve["Fe_H"], curve["A_Li"], label=label, color=color, linestyle=linestyle)
    ...
    axs[1].scatter(feh_plot, ali_payne - 1.05 - feh_plot, s=20, edgecolor='none', c='k')
    axs[1].errorbar(feh_plot, ali_payne - 1.05 - feh_plot, xerr=xerr_plot, yerr=yerr_plot, fmt='none', ecolor='k', elinewidth=0.5, capsize=2)
    axs[1].set_ylabel(r"[Li/Fe]", fontsize=fontsize)
    axs[1].set_xlabel(r"[Fe/H]", fontsize=fontsize)
    axs[1].set_ylim(-0.4, 4.6)
    #axs[1].axhline(0, color='black', linestyle='--', linewidth=0.8)
    #for color, curve in zip(colors, curves):
    #    axs[1].plot(curve["Fe_H"], curve["Li_Fe"], label=f"{color.capitalize()} curve", color=color, linestyle='--')

    axs[0].legend(loc='upper left', fontsize=10, frameon=False, ncol=2, bbox_to_anchor=(-0.03, 1.02),
