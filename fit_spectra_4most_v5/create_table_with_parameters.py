...
if __name__ == '__main__':
    ...
    for row in data.iterrows():
        ...
        total_string = f"{spectraname} & {instrument} & ${row[1]['teff'] * 1000:.0f} \\pm {row[1]['teff_std'] * 1000 + fit_systematic_error.teff_error:.0f}$ & ${row[1]['logg']:.2f} \\pm {row[1]['logg_std'] + fit_systematic_error.logg_error:.2f}$ & ${row[1]['vmic']:.2f} \\pm {row[1]['vmic_std']:.2f}$ & ${row[1]['feh']:.2f} \\pm {row[1]['feh_std'] + fit_systematic_error.feh_error:.2f}$  & ${row[1]['vsini']:.2f} \\pm {row[1]['vsini_std']:.2f}$ "
        for el in elements:
            if el not in row[1] or (el + '_std') not in row[1]:
                ...
                element_value_tsfitpy = row[1][el.replace("_tsfitpy", "")]
