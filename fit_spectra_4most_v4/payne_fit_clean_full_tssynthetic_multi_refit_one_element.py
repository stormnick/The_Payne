...
def fit_one_spectrum(file, stellar_rv, folder, payne_coeffs, wavelength_payne, labels, label_names, resolution_val, payne_data, elements_to_refit):
    ...
    # find correct file in payne_data
    payne_correct_row = payne_data[payne_data['spectraname'] == file]

    final_parameters = {}
    final_parameters_std = {}

    final_parameters["teff"] = payne_correct_row['teff'].values[0]
    final_parameters_std["teff"] = payne_correct_row['teff_std'].values[0]
    final_parameters["logg"] = payne_correct_row['logg'].values[0]
    final_parameters_std["logg"] = payne_correct_row['logg_std'].values[0]
    final_parameters["feh"] = payne_correct_row['feh'].values[0]
    final_parameters_std["feh"] = payne_correct_row['feh_std'].values[0]
    final_parameters["vmic"] = payne_correct_row['vmic'].values[0]
    final_parameters_std["vmic"] = payne_correct_row['vmic_std'].values[0]
    final_parameters["vsini"] = payne_correct_row['vsini'].values[0]
    final_parameters_std["vsini"] = payne_correct_row['vsini_std'].values[0]
    final_parameters["vmac"] = payne_correct_row['vmac'].values[0]
    final_parameters_std["vmac"] = payne_correct_row['vmac_std'].values[0]
    final_parameters["doppler_shift"] = payne_correct_row['doppler_shift'].values[0]
    final_parameters_std["doppler_shift"] = payne_correct_row['doppler_shift_std'].values[0]
...
def _wrapper(path, folder, payne_coeffs, wavelength_payne, labels, label_names, resolution_val, payne_data, elements_to_refit):
    stellar_rv = 0
    return fit_one_spectrum(path, stellar_rv, folder, payne_coeffs, wavelength_payne, labels, label_names, resolution_val, payne_data, elements_to_refit)
...
if __name__ == '__main__':
    ...
    fitted_values.to_csv(f"fitted_benchmark_extended_refitted.csv", index=False)
