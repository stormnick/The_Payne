...
doppler_shift  :      0.327 +/-      0.008"""
    ...
    fitted_values = pd.DataFrame(columns=["spectraname", *labels, "vrot", "vmac", "doppler_shift"])
    ...
    for file in files:
        ...
        print(f"Done fitting in {time_end - time_start:.2f} seconds.")
        ...
        labels_temp.append('vrot')
        labels_temp.append('vmac')
        labels_temp.append('doppler_shift')
        j = 0
        for label, value, input_value in zip(labels_temp, final_params, input_values):
            if input_value is None:
        ...
        new_row_df = pd.DataFrame(
            [[file, *final_params]],
            columns=fitted_values.columns.tolist()
        )
    ...
    fitted_values.to_csv("fitted_values_melchiors.csv", index=False)